...
( function( $ )
{
	...
	$.getJSON("https://cors.io/?https://acoustic-bank.glitch.me/api/users", function(user) {
	var rank = 0; 
	$.when(
	
	$.each(user, function(key, users) { 
	if(users.Username != "Unknown"){
		rank = rank + 1;
		$("<tr><td>" + rank + ".</td><td>" + users.Username + "</td><td>" + users.TotalXp+ "</td><td>" + users.CurrencyAmount + "</td></tr>").appendTo("#leaderboard");
	}
	})).then(function () { 	var dTables = $( '.dTables' );	
		dTables.DataTable(
		{
			responsive: true,
			info: false,
